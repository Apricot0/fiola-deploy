...
import tensorflow as tf  # For graph/session management
...
fio_objects = []
graph = None  # Global variable for TensorFlow graph
sess = None   # Global variable for TensorFlow session
...
def sync_memmap_from_buffer(tiff_buffer):
...
async def memmap_from_buffer(tiff_buffer):
    ...
    return await loop.run_in_executor(None, sync_memmap_from_buffer, tiff_buffer)

def process_frame_data(memmap_image):
    return memmap_image.astype(np.float32)

def safe_predict(model, sess, graph, input_data):
...
async def process_frame_with_buffer(fio, frame_data, frame_idx, timestamp, processtimestamp, local, model):
    ...
    try:
        start_time = time()
        loop = asyncio.get_running_loop()
        memmap_image = await loop.run_in_executor(None, sync_memmap_from_buffer, frame_data)
        buffer_time = time()
        proc_time = time()
        ...
        await loop.run_in_executor(None, fio.fit_online_frame, memmap_image)
        ...
        end_time = time()
        total_time = end_time - timestamp / 1000
        logging.info(f"Total time spent on frame {frame_idx} (from capture to finish): {total_time}, total processing time: {end_time - proc_time}, buffering time: {buffer_time - start_time}, start-to-finish: {end_time - start_time}")
...
async def processing():
    global fio_objects, sender_id, model, graph, sess
    loop = asyncio.get_running_loop()
    loop.set_default_executor(concurrent.futures.ThreadPoolExecutor(max_workers=os.cpu_count()))
    ...
    # Capture the default graph and session for the model
...
def parse_arguments():
    ...
    parser.add_argument("-lt", "--local-test", action="store_true", help="Run the script in local test mode with synthetic data.")
